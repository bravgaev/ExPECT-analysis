...
model_meth_presympt <- getMeth(bs_presympt_filtered, what = "perRegion", type = "smooth", regions = dmr_presympt_finder_train_filtered[1:8,]) %>% 
    t(.) %>%
    as_tibble(.) %>% 
...
test_set_sympt <- getMeth(bs_sympt, type = "smooth", what = "perRegion", regions = dmr_presympt_finder_train_filtered[1:8,]) %>% 
  t(.) %>%
  as_tibble(.) %>% 
...
model_meth_sympt <- getMeth(bs_sympt_filtered, what = "perRegion", type = "smooth", regions = dmr_sympt_finder_train_filtered[1:8,]) %>% 
    t(.) %>%
    as_tibble(.) %>% 
...
test_set_presympt <- getMeth(bs_presympt, type = "smooth", what = "perRegion", regions = dmr_sympt_finder_train_filtered[1:8,]) %>% 
  t(.) %>%
  as_tibble(.) %>% 
